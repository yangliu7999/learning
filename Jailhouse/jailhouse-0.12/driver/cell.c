...
//命令使用：jailhouse cell create configs/arm64/rpi4-inmate-demo.cell
//传入参数为.cell文件
...
//从传入参数中读取.cell的内容至cell_params
	if (copy_from_user(&cell_params, arg, sizeof(cell_params)))
		return -EFAULT;

//用来为用户空间页在内核空间来分配内存，当一个分配无法满足时,阻止内核来发出警告
//给 kmalloc 的第一个参数是要分配的块的大小，第2个参数, 分配标志.
//成功返回分配的目标内存空间的首地址
